...
st.markdown(
    ...
    + "propagating, the light passing through linear polarizer.**"
)
...
with col2:
    ...
    transfer_field1 = cx.linear_polarizer(
        transfer_field1, analyzer_angle_frac * jnp.pi / 2
    )
    fig, ax = plt.subplots(1, 1)
    fig = add_intensity_to_axes(fig, transfer_field1, ax, extent)
    st.pyplot(fig)
